...
{
 	LCD->LCD_REG=lcddev.wramcmd;	  //发送开始写入的指令
}	
/****************************************************
函数名：LCD_WriteRAM
形参：RGB_Code:颜色值
...
{	 
	...
	LCD_WriteRAM_Prepare();	//开始写入GRAM （LCD屏的显示原理？？？？？？？？）
	LCD->LCD_RAM=c; 
}
/****************************************************
函数名：LCD_ReadPoint
...
void LCD_Clear(u16 color)
{
	...
	LCD_SetCursor(0x00,0x0000);	//设置光标（LCD屏的坐标位）位置 
...
void  show_xXx(u32 x ,u32 y,u16 backColor,u16 color,const u8 *ch ,u16 size_line,u16 size_bits)
{
	...
	{                              //0x10,0x02 0x1000 | 0x02 = 0x1002
		...
		for(i=0;i<size_bits/8;i++)    //每次取两个字节，两个字节代表一行，一个数据可以打8个点，每位1代表打前景颜色，0代表打背景颜色
		{
			dat = *ch++;						
			for(x0=0;x0<8;x0++)             //换列
			{
...
void Draw_Text_8_16_Str(u32 x,u32 y,u16 backColor,u16 color,const unsigned char *chs)
{
	...
	while (*chs != '\0')
	{
		if (*chs > 0xa0)				//汉字内码都是大于0xa0  (机内码与区内码之间的转换)
		{
			for (i=0 ;i < GB16_NUM();i++)  //GB16_NUM（）功能是显示汉字库内的个数
			{	
				if(x>235){x=0;y=y+16;}
...
#if LCD_ZK_EN
/****************************************************
函数名：Draw_Text_16_16_Str_zk
形参：x：横坐标   y：纵坐标     color文字颜色  
   	  backColor  文字背景颜色  chs：字模数组
返回值：
函数功能：运用字库显示汉字（16*16）
****************************************************/
#define FONT_SIZE   (16)
...
void Draw_Text_16_16_Str_zk(u32 x,u32 y,u16 backColor,u16 color,const unsigned char *chs)
{
	while (*chs != '\0')
	{
		...
		FLASH_ReadBuffer(dots,zk_addr1,DOTS_BYTES);//读出Flash中的数据
		if(x>235){x=0;y=y+16;}
...
{  
    ...
    static char s[20];      //必须为static变量，或者是全局变量  
