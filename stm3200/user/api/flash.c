...
void FLASH_Init(void)
{
	...
	//SPI协议的初始化
	SPI_InitStructure.SPI_Direction = SPI_Direction_2Lines_FullDuplex;//选择单项或双向的数据模式
	SPI_InitStructure.SPI_Mode = SPI_Mode_Master; //模式选择，选择了主SPI
	SPI_InitStructure.SPI_DataSize = SPI_DataSize_8b; //数据的位数
	SPI_InitStructure.SPI_CPOL = SPI_CPOL_Low; //时钟悬空时为高 
	SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge; //数据获取于第一个时钟沿（下降沿获取数据）
	SPI_InitStructure.SPI_NSS = SPI_NSS_Soft; //内部NSS信号由SSI位控制（软件控制）
...
void FLASH_WriteEnable(void)
{
  ...
  FLASH_CS_LOW();      //CS片选拉低：选中Flash

  /*!< Send "Write Enable" instruction */
  SPI2_Send_Receive_Byte(CMD_WRITE_ENABLE);    //发送指令：写使能
...
void FLASH_WaitForWriteEnd(void)
{
  ...
  SPI2_Send_Receive_Byte(CMD_READ_REGISTER1);   //发送指令：读取状态寄存器
  ...
  do
  {
    ...
    flashstatus = SPI2_Send_Receive_Byte(FLASH_DUMMY_BYTE);        //给Flash发送垃圾值，Flash返回状态寄存器的状态

  }
  while ((flashstatus & 0x01) == SET); /* Write in progress */    //看状态寄存器的第八位，判断状态寄存器是否忙碌（让Flash忙碌一会）
...
void FLASH_WritePage(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
  ...
  SPI2_Send_Receive_Byte(CMD_PAGE_PROGRAM);   //发送页写的指令
...
void FLASH_ReadBuffer(uint8_t* pBuffer, uint32_t ReadAddr, uint16_t NumByteToRead)
{
  ...
  SPI2_Send_Receive_Byte(CMD_READ_DATA);     //发送读取数据的指令
...
uint32_t FLASH_ReadID(void)
{
  ...
  SPI2_Send_Receive_Byte(CMD_READ_ID);    //发送读取Flash的ID的命令

  /*!< Read a byte from the FLASH */
  Temp0 = SPI2_Send_Receive_Byte(FLASH_DUMMY_BYTE);     //同发同收，需发送垃圾值，以接受Flash返回的ID值
  ...
  Temp = (Temp0 << 16) | (Temp1 << 8) | Temp2;          //一次只能接受8位数据，需把三次接受的数据整合在一个变量中
...
{
  /*!< Send write enable instruction */
  FLASH_WriteEnable();             //给Flash发送数据或指令前都需要写使能

  /*!< Sector Erase */
  /*!< Select the FLASH: Chip Select low */
  FLASH_CS_LOW();
  /*!< Send Sector Erase instruction */
  SPI2_Send_Receive_Byte(CMD_SECTOR_ERASE);      //发送扇区擦除的指令
  ...
  /*!< Wait the end of Flash writing */
  FLASH_WaitForWriteEnd();        //判断Flash的BUSY位，让Flash忙碌一会
}
